...
class CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        value_layer = torch.ones([tokens, 1, heads, 128], dtype=torch.float16, device=query_layer.device)
        print(output_size)
        output_size = (1, heads, tokens, tokens)
        attention_mask = torch.tril(torch.ones((1, tokens, tokens), device=query_layer.device)).view(1, 1, tokens, tokens)
        ...
        # attention_scores.masked_fill_(dist_mask, -10000.0)
        ...
        torch.cuda.synchronize()
        attn_time1 = time.time()
        attn_time = (attn_time1 - attn_time0) * 1000
        print(f"Attn time: {attn_time}ms")
        exit()
