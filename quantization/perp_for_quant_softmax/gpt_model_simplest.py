...
class CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        # # window = attention_probs.unfold(3, 128, 128)
        window = attention_probs

        # max_values = window.max(dim=3, keepdim=True).values.fill_(1.0)
        # min_values = window.min(dim=3, keepdim=True).values.fill_(0)
        # scale_values = 1 / 256
        attention_probs_quant = window * 256.0
        attention_probs_quant = torch.where(
            torch.isnan(attention_probs_quant), torch.zeros_like(attention_probs_quant), attention_probs_quant
        )
        attention_probs_quant = torch.round(attention_probs_quant)

        # attention_probs_quant = attention_probs_quant.to(torch.float16)
        attention_probs_quant = attention_probs_quant / 256.0
        attention_probs_quant = torch.where(
            torch.isnan(attention_probs_quant), torch.zeros_like(attention_probs_quant), attention_probs_quant
        )
        attention_probs = attention_probs_quant.view(1, 16, 2048, 2048)

        # window = attention_probs
        # max_values = window.max(dim=3, keepdim=True).values
        # data_abs = torch.abs(window)
        # data_sign = torch.sign(window)
        # log_max = torch.log(max_values + 1)
        ...
        # attention_probs = data.view(1, 16, 2048, 2048).to(torch.float16)
