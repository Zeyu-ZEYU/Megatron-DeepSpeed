...
class _CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer, causal):
        ...
        # )
        matmul_input_buffer = torch.empty(1, dtype=torch.float16, device=query_layer.device)
        ...
        attention_probs = softmax(attention_scores, self.layer_num, causal)
...
class Attention(torch.nn.Module):
    def __init__(self, layer_num, *args, **kwargs) -> None:
        ...
        if FLASH_ATTENTION:
            self.core_attention = _CoreFlashAttention(self.layer_num)
        else:
    ...
    def forward(self, hidden_states, set_inference_key_value_memory=False, inference_max_sequence_len=None):
        ...
        context_layer = self.core_attention(query_layer, key_layer, value_layer, set_inference_key_value_memory)
...
class GPTModel(torch.nn.Module):
    ...
    def _init_model(self, param_pickle_path, synthetic_params=SYNTHETIC_PARAMS):
        if synthetic_params:
            pass
        else:
