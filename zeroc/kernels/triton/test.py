...
def matmul_uint8(a, b):
    ...
    K, N = b.shape
    # Allocates output.
    c = torch.empty((M, N), device=a.device, dtype=torch.uint16)
    # 1D launch kernel where each block gets its own program.
    grid = lambda META: (triton.cdiv(M, META["BLOCK_SIZE_M"]) * triton.cdiv(N, META["BLOCK_SIZE_N"] * 2),)
    matmul_kernel_uint8[grid](
        a,
        b,
        c,  #
        M,
...
torch.manual_seed(0)
scale = 30
a = torch.randint(0, 10, (scale * 1024, scale * 1024), device="cuda", dtype=torch.int8)
b = torch.randint(0, 10, (scale * 1024, scale * 1024), device="cuda", dtype=torch.int8)
triton_output = matmul_uint8(a, b)
for _ in range(10):
    a = torch.randint(0, 10, (scale * 1024, scale * 1024), device="cuda", dtype=torch.int8)
    b = torch.randint(0, 10, (scale * 1024, scale * 1024), device="cuda", dtype=torch.int8)
    with _Timer("test"):
